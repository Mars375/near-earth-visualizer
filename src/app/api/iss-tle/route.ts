...
// Same 2h Celestrak rate limit as /api/hubble and /api/satellites.
...
/**
 * Real TLE for the ISS (NORAD 25544) via Celestrak — propagated client-side
 * with satellite.js/SGP4, same pipeline as Hubble and the Starlink field.
 * Replaces the old wheretheiss.at REST-polling approach (/api/iss), which
 * only refreshed every 15s and made the station's motion visibly choppy;
 * this gives continuous, smooth, real orbital motion every frame instead.
 */
